# Alpaca Trading Bot V7 - Gamma Sniper Edition
# Features:
# - V6 Core (Swing Sentiment, Risk Management)
# - NEW: Options Scalping (0-5 DTE) driven by DayTradingAgent
# - NEW: Mixed Strategy Execution
...
logger = logging.getLogger("HedgeFund_V7")
...
if not logger.handlers:
    ...
    file_handler = logging.FileHandler("trading_bot_v7.log")
...
ENABLE_OPTIONS = False     # Master Control for SWING Options
ENABLE_OPTIONS_SCALP = True # ⚡ NEW: Master Control for SCALP Options (0-5 DTE)
             ...
             if counts['scalp'] < 10:
                 ...
                 if scalp_res['signal'] == 'BUY':
                     ...
                     if ENABLE_OPTIONS_SCALP:
                         ...
                         opt_res = self.options_agent.get_scalp_contract(symbol, type='call')
                         
                         if opt_res:
                             # Execute Option Scalp
                             logger.info(f"⚡ V7 Gamma Sniper: {opt_res['contract_symbol']} (Exp: {opt_res['expiry']})")
...
# =============================================================================
def run_hedge_fund():
    logger.info("Initializing AI Hedge Fund V7 (Gamma Sniper)...")
