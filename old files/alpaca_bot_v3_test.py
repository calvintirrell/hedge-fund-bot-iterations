...
# Corrected Imports for StopLossRequest approach
...
logger.setLevel(logging.INFO) # Change to logging.DEBUG to see debug messages

# Console Handler
stream_handler = logging.StreamHandler(sys.stdout)
stream_handler.setFormatter(log_formatter)
# Avoid adding handler if it already exists (important if script reruns in same session)
...
# --- STRATEGY PARAMETERS ---
STRATEGY_SYMBOL = "IBM"       # << CHANGE THIS TO TEST A NEW SYMBOL
...
def check_strategy(trading_client):
    ...
    """
    global STRATEGY_SYMBOL # Allow modification if needed, though not used here
    try:
        ...
        if (latest_intraday_fast > latest_intraday_slow) and \
           ...
           (latest_rsi < RSI_OVERBOUGHT):
            ...
            market_order_data = MarketOrderRequest(
                                symbol=STRATEGY_SYMBOL, qty=ORDER_QTY,
                                side=OrderSide.BUY, time_in_force=TimeInForce.DAY)
            buy_order = None
            try:
                buy_order = trading_client.submit_order(order_data=market_order_data)
                logger.info(f"Market BUY order submitted for {ORDER_QTY} share(s) of {STRATEGY_SYMBOL}.")
                logger.info(f"  Buy Order ID: {buy_order.id}, Status: {buy_order.status}")

                # --- Place Stop-Loss Order using StopLossRequest (Attempt 3) ---
                # Calculate stop price based on the latest close used for the signal
                stop_price = round(latest_close_price * (1 - STOP_LOSS_PERCENT / 100), 2)
                logger.info(f"Attempting to place stop-loss order at ${stop_price:.2f} using StopLossRequest...")

                # Be explicit: Add notional=None
                stop_loss_data = StopLossRequest(
                                    ...
                                    notional=None # Explicitly add notional=None
                                 )
                try:
                    # Log the request data before submitting
                    logger.debug(f"Submitting StopLossRequest data: {stop_loss_data}")
                    stop_order = trading_client.submit_order(order_data=stop_loss_data)
                    logger.info(f"Stop-Loss SELL order submitted via StopLossRequest at ${stop_price:.2f}.")
                    logger.info(f"  Stop Order ID: {stop_order.id}, Status: {stop_order.status}")
                except Exception as stop_e:
                    logger.error(f"Could not submit stop-loss order via StopLossRequest: {stop_e}")
                    # Log the failed request data for debugging
                    logger.error(f"Failed StopLossRequest data: {stop_loss_data}")
                # ---------------------------
        ...
        # SELL SIGNAL: Intraday cross DOWN + Daily Trend DOWN + Have Position + RSI NOT Oversold
            ...
            try:
                # Use MarketOrderRequest for closing position, more reliable than close_position
                sell_order_data = MarketOrderRequest(
                                    symbol=STRATEGY_SYMBOL,
                                    qty=ORDER_QTY, # Assuming we want to sell the same qty we bought
                                    side=OrderSide.SELL,
                                    time_in_force=TimeInForce.DAY
                                  )
                sell_order = trading_client.submit_order(order_data=sell_order_data)
                logger.info(f"Market SELL order submitted for {ORDER_QTY} shares to close position in {STRATEGY_SYMBOL}.")
                logger.info(f"  Close Order ID: {sell_order.id}, Status: {sell_order.status}")
            except Exception as e:
        ...
        try:
            ...
            orders_request = GetOrdersRequest(
                ...
                limit=10, # Limit results for logging clarity
