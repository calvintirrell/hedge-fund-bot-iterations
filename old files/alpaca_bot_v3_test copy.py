...
def check_strategy(trading_client):
    ...
    try:
        ...
        try:
            ...
            intraday_df.ta.vwap(append=True) # Calculate VWAP
        ...
        vwap_col = 'VWAP_D' # Standard column name from pandas_ta for Daily VWAP

        # Check if all needed indicator columns exist
        required_cols = [intraday_fast_sma_col, intraday_slow_sma_col, rsi_col, vwap_col]
        ...
        latest_vwap = latest_intraday_data[vwap_col]
        ...
        logger.info(f"Intraday VWAP: {latest_vwap:.2f}")
        ...
        # BUY SIGNAL: Intraday cross UP + Daily Trend UP + No Position + RSI NOT Overbought + Price > VWAP
        if (latest_intraday_fast > latest_intraday_slow) and \
           (daily_trend == 'UP') and \
           (not have_position) and \
           (latest_rsi < RSI_OVERBOUGHT) and \
           (latest_close_price > latest_vwap):

            # --- START OF CORRECTED BUY BLOCK ---
            # BUY SIGNAL: Submit as an OTO order (Market Buy + Stop Loss)
            logger.info(f"BUY SIGNAL detected: Intraday SMA crossed UP, Daily Trend UP, RSI < {RSI_OVERBOUGHT}, AND Price (${latest_close_price:.2f}) > VWAP (${latest_vwap:.2f}).")
        ...
        # HOLD SIGNAL: Otherwise
            ...
            elif latest_intraday_fast > latest_intraday_slow and daily_trend == 'UP' and not have_position and latest_close_price <= latest_vwap:
                 logger.info(f"  Reason: Intraday BUY signal ignored, Price (${latest_close_price:.2f}) <= VWAP (${latest_vwap:.2f}).")
